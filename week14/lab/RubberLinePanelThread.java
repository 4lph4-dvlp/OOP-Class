...
class SharedData {
	...
	public synchronized void decTotal(int size) {
		total = total - size;
		for(JPanel p: panelList) {
			p.repaint();
		}
...
public class RubberLinePanelThread extends JPanel 
{
	...
	public RubberLinePanelThread(SharedData d) {
		sdata = d;
		d.regPanel(this);
	...
	private class ClearButtonListener implements ActionListener   {
		 public void actionPerformed (ActionEvent event)      {
			         //추가하시오
			 clear = true;
			 if(sdata != null) sdata.decTotal(plist.size()); 
				 //sdata.total -= plist.size();
		    ...
		    public void mouseReleased(MouseEvent event){
		       ...
		       if(p1 != null && p2 != null) {
		    	 ...
		    	 case RECTANGLE: 
		    		...
		    		p = new Rectangle(p1.x, p1.y,width,height);
		    		break;
		    	 case CIRCLE:
	...
	public void paintComponent(Graphics page) {
			...
			if(sdata != null) page.drawString("total # of polygons: "+ sdata.getTotal(), 0,30);
			Polygon p;
		...
		if ((p1 != null) && (p2 != null)) {
			...
			case RECTANGLE: 
				int width = Math.abs(p2.x - p1.x);
		        int height = Math.abs(p2.y - p1.y);
	    		page.drawRect(p1.x, p1.y, width, height );
	    		break;
	    	default:
			} // end of switch
		} //end of if
